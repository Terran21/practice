...
class Setup:
    ...
    def fit(self, **kwargs):
        ...
        self.opt_version = 'standard'
        if 'version' in kwargs:
            self.opt_version = kwargs['version']
        self.beta = 0
        if 'beta' in kwargs:
            self.beta = kwargs['beta']

        # run gradient descent
        weight_history, cost_history = optimizers.gradient_descent(self.cost, self.alpha_choice, self.max_its,
                                                                   self.w_init, self.beta, self.opt_version)
        ...
        if self.cost_name == 'softmax' or self.cost_name == 'perceptron' or self.cost_name == 'multiclass_softmax' \
